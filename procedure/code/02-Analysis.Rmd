...
hh_capacity_t <- dhshh_2010 %>%
  ...
  # and calculate something for each one
  rowwise %>%
  mutate(hhlivestock = sum(HV246A, HV246D, HV246E, HV246G, na.rm = T)) %>%
  # rowwise() creates a group for every row. must ungroup now:
  ungroup %>%
...
``` {r summary stats for tas at hh level before pulling nas}
...
blank = st_as_stars(st_bbox(b), dx = 0.083333, dy = 0.083333)
blank[[1]][] = NA
...
orfig5rast = st_rasterize(orfig5vect["bg_mean"], template=final)
